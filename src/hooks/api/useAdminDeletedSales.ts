...
export function useAdminDeletedSalesConversions(filters: SalesConversionsFilters) {
  return useQuery({
    queryKey: ['admin-deleted-sales-conversions', filters],
    queryFn: async () => {
      const response = await adminClient.get<AdminSalesConversionSearchResponse>(
        adminEndpoints.salesDeleted.conversions,
        { params: cleanQueryParams(filters) },
      )
      return response.data
    },
