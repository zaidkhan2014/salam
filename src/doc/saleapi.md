...
Self-contained guide for the **standalone sales dashboard** (sales employees). The main admin panel contract is in [`FRONTEND_ADMIN_API_CONTRACT.md`](./FRONTEND_ADMIN_API_CONTRACT.md) (analytics, users, reports, staff CRUD). **Minimum income dropdown values:** [`FRONTEND_SALES_INCOME_BANDS.md`](./FRONTEND_SALES_INCOME_BANDS.md). This document covers everything the sales SPA needs.
...
| `birthYear` | int | 1900–2100 — **maximum birth year** (profiles born in that year **or earlier**). Example: `2000` → `dateOfBirth` ≤ `2000-12-31`. UI label suggestion: “Born on or before (year)”. Invalid years → **400**. |
| `subscribed` | boolean | `true` / `false` |
| `verifiedProfile` | boolean | `true` / `false` |
| `minIncomeBandId` | string | Minimum income — hardcoded band id from [`FRONTEND_SALES_INCOME_BANDS.md`](./FRONTEND_SALES_INCOME_BANDS.md) (e.g. `INC_IN_30_40`). Server converts band **lower bound** to USD and keeps profiles where `careerEducation.incomePerYearUsd >= threshold`. Profiles without income data are excluded. Unknown band → **400**. |
...
| `minIncomeBandId` | Minimum annual income — band id from [`FRONTEND_SALES_INCOME_BANDS.md`](./FRONTEND_SALES_INCOME_BANDS.md) (e.g. `INC_IN_30_40` for “₹30–₹40 lakh” and up) |
...
export interface SalesLeadsFilters {
  ...
  /** Maximum birth year (inclusive) — born on or before Dec 31 of this year */
  birthYear?: number;
  maritalStatus?: string;
  state?: string;
  city?: string;
  /** e.g. INC_IN_30_40 — see FRONTEND_SALES_INCOME_BANDS.md */
...
**Q: How to filter by minimum income?**  
A: Hardcode dropdown options from [`FRONTEND_SALES_INCOME_BANDS.md`](./FRONTEND_SALES_INCOME_BANDS.md). Pass selected band **`id`** as `minIncomeBandId` on `GET /sales/leads`. Example: `minIncomeBandId=INC_IN_30_40` returns profiles with stored `incomePerYearUsd` at or above the ₹30 lakh lower bound (converted to USD server-side).

**Q: What does `birthYear=2000` mean?**  
A: **Maximum birth year** — profiles with `dateOfBirth` on or before `2000-12-31` (born in 2000 or earlier / older age group). Not an exact-year match.
