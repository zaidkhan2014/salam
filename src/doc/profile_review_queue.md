...
```
┌──────────────────────────────────────────────────────────────┐
│ Filters: Gender [Female ▾]  Created for [Self ▾]             │
│          Status [All ▾]  Code [All ▾]  Date range  Page size │
├──────────────────────────────────────────────────────────────┤
│ Row: Name | Gender | Created for | Status | Code | Photos    │
...
1. Load `GET /review-queue?gender=Female` (or Male / omit). Optional: `profileCreatedFor=SELF` or `NON_SELF`.
...
| `profileCreatedFor` | enum | No | — | `SELF` or `NON_SELF` (case-insensitive). Omit = no filter. Other values → **400** |
...
### `profileCreatedFor` filter

| Value | Matches `basicDetails.profileCreatedFor` |
|-------|------------------------------------------|
| `SELF` | Case-insensitive exact `"Self"` |
| `NON_SELF` | Case-insensitive one of: `Son`, `Daughter`, `Brother`, `Sister`, `Other` |

Null / blank / unexpected stored values match **neither** bucket — they only appear when this filter is omitted.

Client UI labels: **Self** → `SELF`, **Non-self** → `NON_SELF`.
...
GET /api/admin/users/review-queue?profileCreatedFor=SELF
GET /api/admin/users/review-queue?profileCreatedFor=NON_SELF&gender=Female
...
| `profileCreatedFor` | string \| null | Raw value from `basicDetails` (e.g. `Self`, `Son`, `Daughter`, …) |
...
type ProfileCreatedForFilter = "SELF" | "NON_SELF";
...
interface AdminReviewQueueFilters {
  gender?: string;
...
| Date | Change |
|------|--------|
| 2026-09-26 | `profileCreatedFor` filter (`SELF` / `NON_SELF`); flattened `profileCreatedFor` on list item |
