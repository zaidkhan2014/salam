...
import {
  useAdminVipLeadDetail,
  useAssignVipLead,
  useClaimVipLead,
  useReleaseVipLead,
  useUpdateVipFollowUp,
  useUpdateVipNote,
  useUpdateVipStatus,
} from '@/hooks/api/useAdminVip'
import {
...
type VipDetailLocationState = {
  vipListSearch?: string
}

function vipListSearchForLink(raw?: string): string {
  if (!raw) return ''
...
function syncDetailForms(data: {
  ...
  return {
    status: data.status as AdminSalesStatus,
...
export default function VipLeadDetailPage() {
  const { userId } = useParams<{ userId: string }>()
  const location = useLocation()
  const { session } = useAuth()
  const state = location.state as VipDetailLocationState | null
  const backSearch = vipListSearchForLink(state?.vipListSearch)

  const query = useAdminVipLeadDetail(userId)
  const updateStatus = useUpdateVipStatus(userId ?? '')
  const updateNote = useUpdateVipNote(userId ?? '')
  const updateFollowUp = useUpdateVipFollowUp(userId ?? '')
  const claimLead = useClaimVipLead(userId ?? '')
  const releaseLead = useReleaseVipLead(userId ?? '')
  const assignLead = useAssignVipLead(userId ?? '')
  ...
  const canClaim =
    query.data &&
    (query.data.status === 'CALL_REMAINING' || query.data.status === 'IN_PROCESS') &&
  ...
  return (
    <section className="min-w-0 space-y-4">
      <PageHeader title="VIP Lead Detail" description={`Manage VIP consultation lead for ${userId ?? '--'}.`} />
      <Link to={{ pathname: routes.vip, search: backSearch }} className="text-sm text-slate-700 underline">
        Back to VIP Customers
      </Link>
        ...
        <div className="min-w-0 space-y-4">
          ...
          <div className="flex min-w-0 flex-col gap-6 lg:flex-row lg:items-start">
            ...
            <div className="min-w-0 flex-1 space-y-4">
              <Card>
                <CardHeader>
                  <CardTitle>VIP lead</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2 text-sm">
                  <Row label="User ID" value={<UserLink userId={query.data.profile.userId} />} />
                  <Row label="Status" value={query.data.status} />
                  <Row label="VIP plan" value={query.data.vipTitle ?? '--'} />
                  <Row label="CTA label" value={query.data.ctaLabel ?? '--'} />
                  <Row label="Request count" value={String(query.data.requestCount ?? '--')} />
                  <Row label="Discounted price" value={formatInr(query.data.discountedPriceInr)} />
                  <Row label="Original price" value={formatInr(query.data.originalPriceInr)} />
                  <Row label="Currency" value={query.data.currency ?? '--'} />
                  <Row label="Assigned to" value={query.data.assignedToAdminId ?? '--'} />
                  <Row label="Claimed at" value={formatDateTime(query.data.claimedAt)} />
                  <Row label="Requested at" value={formatDateTime(query.data.requestedAt)} />
                  <Row label="Last requested" value={formatDateTime(query.data.lastRequestedAt)} />
                  ...
                  <Row label="Lead created" value={formatDateTime(query.data.createdAt)} />
                  <Row label="Lead updated" value={formatDateTime(query.data.updatedAt)} />
                </CardContent>
              </Card>
            </div>
          </div>
