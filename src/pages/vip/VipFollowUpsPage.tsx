...
import { useAdminVipFollowUps } from '@/hooks/api/useAdminVip'
...
export default function VipFollowUpsPage() {
  ...
  const query = useAdminVipFollowUps(filters)
  const totalPages = Math.max(1, Math.ceil((query.data?.total ?? 0) / PAGE_SIZE))

  return (
    <section className="min-w-0 space-y-4">
      <PageHeader title="VIP Follow-ups" description="Follow-up queue for VIP consultation leads." />
      <VipSubNav />
              ...
              <tbody>
                {query.data.items.map((lead) => (
                  <tr key={lead.userId} className="border-t border-slate-100">
                    ...
                    <td className="px-3 py-2">
                      <Link className="underline" to={routes.vipLeadDetail(lead.userId)}>
                        Open
                      </Link>
                    </td>
                  </tr>
